...
int i;
 ...
 sfShape ** generate_points(char * fn, int midPosX, int midPosY)
 {
     ...
     for (; i <= 600; i++, j++)
     {
         ...
         iU_reinit();
         iU_add_variable("x", (i-300+midPosX));
         ...
         iU_reinit();
         iU_add_variable("x", (i-300.0+1+midPosX));
         parse_tree = iU_tokenize(fn);
         iU_gen_parsetables();
 ...
 int main()
 {
    ...
    // Our input
     ...
     sfShape ** x = generate_points(ch, midPosX, midPosY);
     ...
     sfShape * tmpLine;
     char * tmpStr;
     sprintf(tmpStr, "%d, %d", midPosX, midPosY);
