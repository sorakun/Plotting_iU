...
sfShape ** generate_points(char * fn, int midPosX, int midPosY, float zoom)
{
    ...
    for (; i <= 600; i++, j++)
    {
        ...
        iU_reinit();
        iU_add_variable("x", (i-300)/zoom +midPosX);
        ...
        iU_reinit();
        iU_add_variable("x", (i-300.0+1)/zoom+midPosX);
        parse_tree = iU_tokenize(fn);
        iU_gen_parsetables();
        ...
        if(res<0)
            res = abs(res)+300+midPosY;
        else
            res = 300-res+midPosY;

        if(res2<0)
            res2 = abs(res2)+300+midPosY;
        else
            res2 = 300-res2+midPosY;
...
int main()
{
    ...
    sfString_SetColor(Text, sfBlue);
    sfString_SetPosition(Text, 30, 520);
    sfString_SetSize(Text, 30);

    Pos = sfString_Create();
    sfString_SetFont(Pos, Font);
    sfString_SetColor(Pos, sfBlack);
    sfString_SetPosition(Pos, 10, 10);
    sfString_SetSize(Pos, 30);
    ...
    sfShape ** x = generate_points(ch, midPosX, midPosY, zoom);
    AxeX = sfShape_CreateLine(300-midPosX, 0, 300-midPosX, 600, 2, sfBlack, 0, sfBlack);
    AxeY = sfShape_CreateLine(0, 300+midPosY, 600, 300+midPosY, 2, sfBlack, 0, sfBlack);

    sfShape * tmpLine;
    char * tmpStr;
    sprintf(tmpStr, "pos: %d, %d; zoom: %f", midPosX, midPosY, zoom);
    sfString_SetText(Pos, tmpStr);
    /* Start the game loop */
    while (sfRenderWindow_IsOpened(App))
    {
        ...
        for(i = 0; i < 600; i+= 10)
        {
            tmpLine = sfShape_CreateLine(i*zoom, 0, i*zoom, 600, 1, grid, 0, grid);
            sfRenderWindow_DrawShape(App, tmpLine);
            tmpLine = sfShape_CreateLine(0, i*zoom, 600, i*zoom, 1, grid, 0, grid);
            sfRenderWindow_DrawShape(App, tmpLine);
        }
        ...
        sfRenderWindow_DrawString(App, Pos);
